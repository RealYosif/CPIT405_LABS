...
// 3. Populate the Team Filter Dropdown Dynamically
function populateTeams() {
    // Extract unique team names from the data
    const uniqueTeams = [...new Set(data.map(player => player.team))];
...
function filterData() {
    ...
    currentData = data.filter(player => {
...
headers.forEach((header, index) => {
    header.addEventListener('click', () => {
        // Map the column index to the corresponding object property
        const properties = ['name', 'team', 'points', 'rebounds', 'assists'];
        ...
        currentData.sort((a, b) => {
            ...
            // Handle string sorting (Name, Team) vs number sorting (Points, Rebounds, Assists)
        ...
        sortDirection = !sortDirection; // Toggle direction for the next click
        renderTable(currentData);
    });
});
